...
def _challenge(email: str, message: str) -> OtpChallenge:
    return OtpChallenge(email=email, expires_in_seconds=otp.issue(email), message=message)
...
def signup(payload: UserCreate):
    ...
    return _challenge(payload.email, "Verification code sent. Enter it to finish creating your account.")
...
def login(payload: LoginRequest):
    ...
    return _challenge(user["email"], "Verification code sent. Enter it to sign in.")
...
def send_otp(payload: OtpSendRequest):
    ...
    return _challenge(user["email"], "A new verification code has been sent.")
